...
import { StudentPortfolio, Department, Zone, LTC, DashboardConfig } from './types';
import { NGO_INFO, ZONES, LTCS, DEPARTMENTS, INITIAL_DASHBOARD_CONFIG, INITIAL_STUDENTS } from './constants';
    ...
    <div className="min-h-screen bg-red-50/30 selection:bg-red-100">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex gap-1 mb-10 bg-white p-1.5 rounded-2xl border border-red-100 shadow-sm inline-flex overflow-x-auto no-scrollbar max-w-full">
          ...
          <NavTab label="Students" active={viewMode === 'List'} onClick={() => setViewMode('List')} icon="fa-users" />
        ...
        {viewMode === 'Home' && (
          <DashboardHome 
            ...
            onAction={(type) => {
               ...
               else if (type === 'reports') alert("Feature coming soon: Impact PDF Engine");
            }}
          />
        )}
            ...
            <div className="space-y-6 mb-12">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-2 relative">
                  ...
                  <input type="text" placeholder="Search students..." className="w-full pl-14 pr-6 py-4 bg-white border-2 border-red-50 rounded-2xl outline-none focus:border-red-500 shadow-lg shadow-red-100/30 font-bold" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
                <SelectFilter value={zoneFilter} onChange={setZoneFilter} options={zones} label="All Zones" />
                <SelectFilter value={ltcFilter} onChange={setLtcFilter} options={availableLTCsForFilter} label="All Centers" />
              </div>

              <div className="flex overflow-x-auto pb-4 no-scrollbar gap-3 -mx-4 px-4">
                <FilterTab label="All Skills" active={activeFilter === 'All'} onClick={() => setActiveFilter('All')} />
                {departments.map(dept => (
                  <FilterTab key={dept.id} label={dept.name} active={activeFilter === dept.id} onClick={() => setActiveFilter(dept.id)} icon={dept.icon} />
                ))}
              </div>
            </div>
        ...
        {viewMode === 'Dashboard' && isAdmin && (
          <div className="space-y-8 animate-in fade-in duration-500">
             <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">{dbConfig.customTitle}</h2>
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <SummaryCard label="Enrolled" value={portfolios.length} color="bg-red-600" icon="fa-users" />
                <SummaryCard label="Zones" value={zones.length} color="bg-orange-600" icon="fa-layer-group" />
                <SummaryCard label="Centers" value={ltcs.length} color="bg-blue-600" icon="fa-building" />
                <SummaryCard label="Domains" value={departments.length} color="bg-indigo-600" icon="fa-tags" />
             </div>
          </div>
        )}
      </main>
...
const FilterTab = ({ label, active, onClick, icon }: any) => (
  <button onClick={onClick} className={`whitespace-nowrap px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border-2 flex items-center gap-2 ${active ? 'bg-red-600 border-red-600 text-white shadow-md' : 'bg-white text-slate-400 border-red-50'}`}>
...
const SummaryCard = ({ label, value, color, icon }: any) => (
  <div className={`${color} p-8 rounded-[2.5rem] text-white flex items-center justify-between transition-all hover:scale-105 shadow-xl shadow-current/20`}>
     <div><p className="text-[10px] font-black uppercase opacity-80">{label}</p><p className="text-4xl font-black">{value}</p></div>
     <div className="w-16 h-16 bg-white/20 rounded-3xl flex items-center justify-center text-3xl"><i className={`fas ${icon}`}></i></div>
  </div>
);
